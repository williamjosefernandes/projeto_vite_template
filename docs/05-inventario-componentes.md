...
| **Checkbox** | `components/ui/Checkbox` (wrapper `@radix-ui/react-checkbox`) | `size`: `sm`·`md` (padrão)·`lg`. Estado via `checked` (`true` \| `false` \| `"indeterminate"`) | Props do `Checkbox.Root` + `size`, `error` | `<Checkbox checked={v} onCheckedChange={setV} />` |
| **CheckboxField** | `components/ui/Checkbox` | Mesmas de `Checkbox` | `id: string`, `label: ReactNode`, `description?: ReactNode`, + props de `Checkbox` | `<CheckboxField id="terms" label="Aceito os termos" checked={v} onCheckedChange={setV} />` |
...
| **Container / Grid / Stack / Flex / Spacer / Divider** | `components/ui/layout-primitives` | Ver `docs/ds-10-layout.md` | `Grid`: `cols?`, `gap?`. `Stack`: `gap?`, `align?`. `Flex`: `direction?`, `justify?`, `align?`, `gap?`, `wrap?`. `Divider`: `variant?`, `orientation?` | `<Grid cols={12} gap="md">...</Grid>` |
| **ResizablePanel / SplitPane** | `components/ui/resizable-panel` | Nenhuma | `ResizablePanel`: `first`, `second`, `direction?`, `defaultSize?`, `minSize?`, `maxSize?`. `SplitPane`: `sidebar`, `content`, `defaultSidebarSize?` | `<ResizablePanel first={<A />} second={<B />} />` |
| **ScrollArea** | `components/ui/ScrollArea` (wrapper `@radix-ui/react-scroll-area`) | Nenhuma | Props do `Root` + `viewportClassName?` | `<ScrollArea className="h-64">...</ScrollArea>` |
...
| **ImageUpload** | `components/ui/ImageUpload` | `shape`: `square` (padrão)·`circle` | `label: string`, `helperText?`, `icon: ReactNode`, `accept?`, `onFileSelect?: (file: File \| null) => void` | `<ImageUpload label="Enviar foto" icon={<Camera />} shape="circle" onFileSelect={setFile} />` |
...
| **OtpInput** | `components/ui/OtpInput` | Nenhuma | `length?` (padrão 6), `value: string`, `onChange`, `onComplete?`, `disabled?` | `<OtpInput value={code} onChange={setCode} onComplete={handleVerify} />` |
...
| **Steps** | `components/ui/Steps` | Nenhuma — estado (completed/active/upcoming) derivado do índice | `steps: StepItem[]` (`{id, label}`), `currentIndex: number` | `<Steps steps={stepsConfig} currentIndex={2} />` |
