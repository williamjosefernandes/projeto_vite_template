...
| **RadioGroup** | `components/ui/Radio` (wrapper `@radix-ui/react-radio-group`) | Nenhuma | Compound: `RadioGroup` (`Root`, props `value`/`defaultValue`/`onValueChange`), `RadioGroup.Item` (`value`) | `<RadioGroup defaultValue="b"><label><RadioGroup.Item value="a" /> A</label><label><RadioGroup.Item value="b" /> B</label></RadioGroup>` |
