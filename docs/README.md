...
| [`08-autenticacao.md`](./08-autenticacao.md) | Infraestrutura de autenticação (`src/auth`): login real, tokens/refresh, sessão persistida, guards, multi-conta, permissões/menus/componentes |
| [`09-integracao-usuarios-admin-e-cadastro.md`](./09-integracao-usuarios-admin-e-cadastro.md) | Cadastro (`/cadastro`) ligado a `register`/`verify-email`/`resend-verification`; administração de usuários (`/configuracoes/usuarios`) ligada a `src/users`; `Table` em modo servidor; inconsistências encontradas no backend |
| [`10-onboarding-cadastro.md`](./10-onboarding-cadastro.md) | Onboarding completo (Cliente/`Customer` e Empresa/`Company`): schema (`OnboardingDraft`), módulo `onboarding`/`geo` no backend, rascunho persistido por step (não `localStorage`), guards de rota para sessão sem conta ainda, bugs encontrados só em teste de navegador real |
...
| Fase 8 — Autenticação | Login/refresh/sessão real via API, guards, multi-conta | [`08-autenticacao.md`](./08-autenticacao.md) | ✅ concluída |
| Fase 9 — Integração Cadastro + Usuários | Cadastro real (register/verify-email) e administração de usuários (`/configuracoes/usuarios`) | [`09-integracao-usuarios-admin-e-cadastro.md`](./09-integracao-usuarios-admin-e-cadastro.md) | ✅ concluída |
| Fase 10 — Onboarding completo | Cadastro Cliente/Empresa ligado de ponta a ponta ao backend (schema + `onboarding`/`geo`), rascunho por step, criação real de `Account`/`Membership`/`Customer`\|`Company`/`Address` | [`10-onboarding-cadastro.md`](./10-onboarding-cadastro.md) | ✅ concluída |
| Fase 11+ — Módulos futuros | Cada novo módulo/página a partir de um print anexado (Cadastros, Financeiro, Comunicação, Operações, Marketing, Configurações▸Geral) — sem API de backend hoje | um novo `docs/1N-<nome-do-modulo>.md` por módulo | ⏳ pendente |
