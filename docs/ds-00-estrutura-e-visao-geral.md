...
- **`DesignSystemSidebar`** (`src/components/layout/DesignSystemSidebar.tsx`) — cabeçalho "Sua Marca / Design System", campo de busca (`Buscar componentes... ⌘K` — busca ainda não funcional, é placeholder visual), grupos **Fundamentos** e **Componentes**, e rodapé com o toggle de tema (reaproveita `useTheme`, o mesmo hook do portal).
