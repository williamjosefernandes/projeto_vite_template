...
### 3.3 Duas inconsistências descobertas no backend (documentadas, não corrigidas — fora de escopo)

1. **`GET /v1/users` não tem `@ApiStandardResponse`** no Swagger — o shape real do item de `content[]` foi tirado de `users.service.ts::listUsers` (um `Membership` com `user` e `profile` aninhados, não um `User` simples). Tipado em `src/users/types/admin-user.types.ts` a partir do `select`/`include` real do Prisma, não do Swagger.
2. **`UpdateUserStatusDto.status` é documentado/validado como enum `UserStatus`** (`PENDING_EMAIL|ACTIVE|BLOCKED|SUSPENDED`), mas `users.service.ts::updateUserStatus` grava esse valor direto em `Membership.status` (enum `MembershipStatus`: `ACTIVE|INVITED|SUSPENDED|REMOVED`, sem `BLOCKED`/`PENDING_EMAIL`) via `as any`. O frontend segue o contrato documentado (`UserStatus`, é o que o DTO valida) e mostra um aviso (`Alert` amarelo) no modal "Alterar status" avisando que `PENDING_EMAIL`/`BLOCKED` tendem a falhar no servidor. **Recomendação para o backend**: alinhar o enum do DTO com `MembershipStatus`, ou separar dois endpoints/DTOs (status do usuário vs. status do vínculo).
3. **`CreateUserAdminDto.profileId` não tem endpoint de listagem** (não existe `GET /profiles` nem equivalente em nenhum controller). O campo é um `Input` de texto livre (UUID) no modal de criação, com nota explicativa. **Recomendação para o backend**: expor `GET /profiles` (escopado à conta ativa) para virar um `Select`.
